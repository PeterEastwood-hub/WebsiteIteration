...
/**
 * Inline (no defer): renames the engineering stream nav link and tab title as soon as the parser
 * reaches the end of `<body>`. Cursor’s Simple Browser and some static hosts cache or reorder
 * deferred scripts; this keeps “Communities” visible without relying on mirror-stripe-nav.js.
 */
const COMMUNITIES_NAV_SYNC_SCRIPT = `
<script id="nf-communities-nav-sync">
(function () {
  function sync() {
    try {
      document.querySelectorAll(
        'a[href="engineering.html"], a[href="insights-engineering-community.html"], ' +
          'a[href="insights-explore-iter8-engineering.html"], a[href="insights-explore-iter9-engineering.html"]',
      ).forEach(function (a) {
        var t = (a.textContent || '').replace(/\\s+/g, ' ').trim();
        if (!t) return;
        if (/engineering/i.test(t) && !/communities/i.test(t)) a.textContent = 'Communities';
      });
      var ti = document.title || '';
      if (/engineering community/i.test(ti)) {
        document.title = ti.replace(/engineering community/gi, 'Communities');
      } else if (/^\\s*Engineering\\s*\\|/i.test(ti)) {
        document.title = ti.replace(/^\\s*Engineering\\s*\\|/i, 'Communities |');
      } else if (/^\\s*Engineering\\s*—\\s*Iteration/i.test(ti)) {
        document.title = ti.replace(/^\\s*Engineering\\s*—/i, 'Communities —');
      }
    } catch (e0) {}
  }
...
/**
 * After flatten: restore a compact Insights flyout (All insights + Communities + Content hub).
 */
function upgradeInsightsNavWithEngineeringDropdown($) {
  const $a = $('ul[data-testid="header-center"] > li > a[href="insights.html"]').first();
  if (!$a.length) return;
  const $li = $a.parent();
  if ($li.children('div.group.relative').length) {
    $li.find('a[href="engineering.html"]').text('Communities');
    return;
  }
...
/** Desktop + mobile header: Communities stream (all URLs that surface the technical listing). */
function renameEngineeringNavLabelToCommunities($) {
  $(
    'header a[href="engineering.html"], ' +
      'header a[href="insights-engineering-community.html"], ' +
      'header a[href="insights-explore-iter8-engineering.html"], ' +
      'header a[href="insights-explore-iter9-engineering.html"]',
  ).each((_, el) => {
    const $el = $(el);
    const t = ($el.text() || '').replace(/\s+/g, ' ').trim();
    if (!t) return;
    if (/engineering/i.test(t) && !/communities/i.test(t)) $el.text('Communities');
  });
}

/** Serialized HTML: shared Next/RSC payloads repeat “Engineering community” in every page shell. */
function scrubEngineeringCommunityStreamBranding(html) {
  return String(html)
    .replace(/Engineering community/gi, 'Communities')
    .replace(/Engineering\\ncommunity/g, 'Communities')
    .replace(/>Browse Engineering →</g, '>Browse Communities →<')
    .replace(/Explore Engineering →/g, 'Explore Communities →')
    .replace(
      /Deep-dives, tutorials and open source releases from our engineering team\./g,
      'Deep-dives, tutorials and open source releases from our communities.',
    )
    .replace(
      /(<h3 class="nf-article-explore-gate__path-h">)More Engineering(<\/h3>)/g,
      '$1Communities$2',
    )
    .replace(
      /aria-label="Engineering — open technical articles"/g,
      'aria-label="Communities — open technical articles"',
    )
    .replace(
      /<span class="nf-insights-jump-to-community__label">Engineering →<\/span>/g,
      '<span class="nf-insights-jump-to-community__label">Communities →</span>',
    );
}

/** Iteration 8/9 Insights: footer “Looking for technical content?” cross-stream strip only. */
function removeInsightsCrossStreamTechnicalCta($) {
  $('section.nf-cross-stream-cta--to-engineering').remove();
}

/** Iteration 8/9 Insights listing: hero pill to the technical stream is “Communities →”. */
function relabelInsightsExploreIter89InsightsHeroStreamCta($, relPath) {
  const norm = relPath.split(path.sep).join('/');
  const base = path.posix.basename(norm);
  if (base !== 'insights-explore-iter8.html' && base !== 'insights-explore-iter9.html') return;

  $('a.nf-insights-jump-to-community[href$="-engineering.html"]').each((_, el) => {
    const $a = $(el);
    const href = ($a.attr('href') || '').trim();
    if (!/-iter[89]-engineering\.html$/i.test(href)) return;
    const $lab = $a.find('.nf-insights-jump-to-community__label').first();
    if ($lab.length) {
      const labText = ($lab.text() || '').replace(/\s+/g, ' ').trim();
      if (/engineering/i.test(labText) && !/communities/i.test(labText)) {
        const hasArrow = /[→↗]/.test(labText);
        $lab.text(hasArrow ? 'Communities →' : 'Communities');
      }
    }
    const aria = ($a.attr('aria-label') || '').trim();
    if (/^engineering\s*—/i.test(aria)) {
      $a.attr('aria-label', aria.replace(/^engineering\s*—/i, 'Communities —'));
    } else if (/engineering community/i.test(aria)) {
      $a.attr('aria-label', aria.replace(/engineering community/gi, 'Communities'));
    }
  });
}

/** Iteration 8/9 engineering listing: hero h1 uses per-letter spans (same length as “Communities”). */
const H1_INSIGHTS_EXPLORE_COMMUNITIES_SPANNED = `<span class="relative block"><span class="sr-only select-none">Communities</span><span class="absolute w-full z-10" aria-hidden="true"><span>C</span><span>o</span><span>m</span><span>m</span><span>u</span><span>n</span><span>i</span><span>t</span><span>i</span><span>e</span><span>s</span></span><span class="opacity-0 select-none" aria-hidden="true"><span>C</span><span>o</span><span>m</span><span>m</span><span>u</span><span>n</span><span>i</span><span>t</span><span>i</span><span>e</span><span>s</span></span></span>`;

function relabelInsightsExploreEngineeringListings($, relPath) {
  const norm = relPath.split(path.sep).join('/');
  const base = path.posix.basename(norm);
  if (base !== 'insights-explore-iter8-engineering.html' && base !== 'insights-explore-iter9-engineering.html')
    return;

  const $h1 = $('main h1#engineering-community').first();
  if ($h1.length) $h1.html(H1_INSIGHTS_EXPLORE_COMMUNITIES_SPANNED);

  $('p.nf-explore-iter89-engineering-hero-sub').first().each((_, el) => {
    $(el).text(
      'Tutorials, performance notes, protocol walkthroughs and open source releases from people across Nearform communities, shipping in production. Practical detail you can lift into your own systems and repos.',
    );
  });

  $('div[role="tablist"][aria-label="Browse engineering articles by topic"]').attr(
    'aria-label',
    'Browse technical articles by topic',
  );

  $('button[data-nf-tab-topic="engineering-practices"]').each((_, el) => {
    const $b = $(el);
    const t = ($b.text() || '').replace(/\s+/g, ' ').trim();
    if (/^engineering practices$/i.test(t)) $b.text('Communities practices');
  });
}

function relabelEngineeringExploreIterationTitles($) {
  $('title').each((_, el) => {
    const $t = $(el);
    const text = ($t.text() || '').trim();
    if (!/^Engineering\s*—\s*Iteration/i.test(text)) return;
    $t.text(text.replace(/^Engineering\s*—/i, 'Communities —'));
  });
}

/**
 * Engineering community listing: align visible hero + document title with “Communities”.
 */
function relabelEngineeringCommunityLandingPage($, relPath) {
  const norm = relPath.split(path.sep).join('/');
  if (path.posix.basename(norm) !== 'insights-engineering-community.html') return;

  $('title').first().text('Communities | Nearform');
  $('meta[name="description"]').attr(
    'content',
    'Communities — technical articles, tooling, and practice from Nearform.',
  );

  const $h1 = $('main h1#engineering-community').first();
  if (!$h1.length) return;
  const newInner = `<span class="relative block"><span class="sr-only select-none">Communities</span><span class="absolute w-full z-10" aria-hidden="true"><span>C</span><span>o</span><span>m</span><span>m</span><span>u</span><span>n</span><span>i</span><span>t</span><span>i</span><span>e</span><span>s</span><span class="animate-blink animate-blink text-nf-green" aria-hidden="true">_</span></span><span class="opacity-0 select-none" aria-hidden="true"><span>C</span><span>o</span><span>m</span><span>m</span><span>u</span><span>n</span><span>i</span><span>t</span><span>i</span><span>e</span><span>s</span><span>_</span></span></span>`;
  $h1.empty();
  $h1.html(newInner);

  const $heroSec = $('main > section').first();
  const $lead = $heroSec.find('p.mb-6').first();
  if ($lead.length) {
    $lead.text(
      'Hands-on articles from practitioners shipping in production — APIs, frontends, data, platforms, and AI. Written for people who learn in community, not in silos.',
    );
  }

  $('div[role="tablist"][aria-label="Browse insights by topic"]').attr(
    'aria-label',
    'Browse communities by topic',
  );

  const $cta = $heroSec.find('a.nf-insights-jump-to-community').first();
  if ($cta.length) {
    $cta.attr('href', '#nf-insights-pill-panel');
    $cta.attr('aria-label', 'Jump to topics and article list on this page');
    const $lab = $cta.find('.nf-insights-jump-to-community__label').first();
    if ($lab.length) $lab.text('Browse articles');
    const $chev = $cta.find('.nf-insights-jump-to-community__chev').first();
    if ($chev.length) $chev.text('↓');

    const $ctaShell = $cta.closest('div.group.relative.overflow-hidden.inline-flex');
    if ($ctaShell.length && !$ctaShell.find('[data-nf-communities-secondary-cta]').length) {
      const $wrap = $(
        '<span class="relative inline-block transition-transform duration-300 ease-in-out ml-6 md:ml-8"></span>',
      );
      $wrap.append(
        $('<a></a>')
          .attr('href', 'insights.html')
          .attr('data-nf-communities-secondary-cta', '1')
          .attr(
            'class',
            'font-sans text-base/4 text-nf-deep-navy dark:text-white underline underline-offset-[0.25em] decoration-nf-green/70 hover:decoration-nf-green',
          )
          .text('Strategy & AI on Insights'),
      );
      $ctaShell.append($wrap);
    }
  }
}
...
function stripAndAnnotate(html, relPath = '') {
  ...
  renameEngineeringNavLabelToCommunities($);
  relabelEngineeringExploreIterationTitles($);
  relabelInsightsExploreEngineeringListings($, relPath);
  relabelInsightsExploreIter89InsightsHeroStreamCta($, relPath);
  removeInsightsCrossStreamTechnicalCta($);
  relabelEngineeringCommunityLandingPage($, relPath);
  ensureFlairAssets($, relPath);
  if ($('#nf-communities-nav-sync').length === 0) {
    $('body').append(COMMUNITIES_NAV_SYNC_SCRIPT);
  }
  if ($('#ux-audit-bar').length === 0) {
    $('body').append(BAR_HTML);
  }
  let out = $.root().html();
  out = scrubEngineeringCommunityStreamBranding(out);
  return out;
}
